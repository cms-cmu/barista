...
Tested by `src/tests/roast.py` (29 unit tests, stdlib only, no ssh or cluster) in the `code_roast_barista` CI job, which extends nothing and so reports in seconds: it asserts the phase table points at Snakefiles that exist, that every generated remote script parses under `bash -n`, that the snakemake command line keeps the flags a run depends on (`--configfile`, `--config roast_id`, `--printshellcmds`, and `--unlock`/`--rerun-incomplete` only on resume), that the launcher body stays wrapped in `main()`, that the publish/archive selection rules merge in the right order, and it executes the status script against a fake checkout to check a dead driver reads as `error` or `stalled` rather than `running`.
