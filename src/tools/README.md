...
Tested by `src/tests/roast.py` (29 unit tests, stdlib only, no ssh or cluster) in the `code_roast_barista` CI job, which extends nothing and so reports in seconds: it asserts that every generated remote script parses under `bash -n`, that the snakemake command line keeps the flags a run depends on (`--configfile`, `--config roast_id`, `--printshellcmds`, and `--unlock`/`--rerun-incomplete` only on resume), that the launcher body stays wrapped in `main()`, that the publish/archive selection rules merge in the right order, and it executes the status script against a fake checkout to check a dead driver reads as `error` or `stalled` rather than `running`. The phase table is also checked against the Snakefiles on disk, which needs the coffea4bees clone, so that one class runs in your working tree and is skipped in the fast CI job.
...
`status` reports each step as `not started`, `running` (a snakemake driver process is alive), `error` (the driver died after a job failed), `stalled` (died with no error) or `exit=N`, plus snakemake progress and the last log line. It also lists this roast's batch jobs, matched by the scheduler's record of the submitting directory: HTCondor batches by state on cmslpc, and on falcon each slurm job with its rule name, state, elapsed/limit, node, cpus/mem/gres and the last line of that job's own slurm log (training loss, batch counter), followed by jobs that finished in the last two days and a one-line cluster summary. Slurm jobs are grouped per rule so a retried rule shows the attempt that replaced the failure, annotated with how many attempts it took, rather than the stale failures.

Ids are `<label>_<YYYYMMDD>_<barista7>-<coffea4bees7>`; any unique prefix works. The configured cmslpc target is the `cmslpc-el9` gateway: `checkout` records whichever interactive node the load balancer gives, and every later command for that roast follows that record, since only the driver and its tmux window are node-bound. A dead node is re-pinned automatically on the next submit or resume. A `host_file` (e.g. `~/.cmslpc-claude-host`) still overrides the configured target for roasts that have not been checked out yet. Stdlib only.
