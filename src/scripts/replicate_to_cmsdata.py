...
ERA_CONFIG = {
    "Run2": {
        ...
        "ymls": [
            ...
            "2024_v2/data.yml",
            "2024_v2/GluGluToHHTo4B.yml",
            "2024_v2/TT.yml",
            "2024_v2/ZH4b.yml",
            "2024_v2/ZZ4b.yml",
            "2024_v2/synthetic_datasets.yml",
            "2024_v2/friends_HH4b_lowpt.yml",
        ],
...
def strip_at_suffix(path):
    """Strip @@key suffix used in friend tree references (e.g. file.json@@branch)."""
    return path.split("@@")[0] if isinstance(path, str) and "@@" in path else path


def is_fnal(path):
    return isinstance(path, str) and strip_at_suffix(path).startswith("root://cmseos.fnal.gov")
...
def collect_from_node(node, n_samples, result):
    ...
    for key, val in node.items():
        ...
        if key == "files":
            for f in (val or []):
                if is_fnal(f):
                    result.add(strip_at_suffix(f))
        elif key == "files_template":
            for tmpl in (val or []):
                if is_fnal(tmpl):
                    for f in expand(strip_at_suffix(tmpl), n_samples):
                        result.add(f)
        elif key in FVT_FILE_KEYS:
            if is_fnal(val):
                for f in expand(strip_at_suffix(val), n_samples):
                    result.add(f)
        elif isinstance(val, str) and is_fnal(val):
            # Plain string value (e.g. friend tree references like FvT, SvB_MA)
            result.add(strip_at_suffix(val))
        else:
