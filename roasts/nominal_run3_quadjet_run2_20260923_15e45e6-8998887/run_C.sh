#!/usr/bin/env bash
# roast nominal_run3_quadjet_run2_20260923_15e45e6-8998887 step C -- generated 2026-09-23 21:30:16
...
EXIT=logs/C.exit
rm -f "$EXIT"
echo "=== roast nominal_run3_quadjet_run2_20260923_15e45e6-8998887 step C start $(date) on $(hostname) ===" | tee -a "$LOG"
...
./run_container snakemake -s coffea4bees/workflows/Snakefile_PhaseC.smk --configfile roasts/nominal_run3_quadjet_run2_20260923_15e45e6-8998887/config.yml --cores 4 --jobs 4 --printshellcmds --config roast_id=nominal_run3_quadjet_run2_20260923_15e45e6-8998887 -n 2>&1 | tee -a "$LOG"
RC=${PIPESTATUS[0]}
echo "=== roast nominal_run3_quadjet_run2_20260923_15e45e6-8998887 step C exit $RC $(date) ===" | tee -a "$LOG"
echo "$RC" > "$EXIT"
if [ "$RC" != "0" ]; then
