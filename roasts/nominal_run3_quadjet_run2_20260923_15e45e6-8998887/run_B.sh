#!/usr/bin/env bash
# roast nominal_run3_quadjet_run2_20260923_15e45e6-8998887 step B -- generated 2026-09-23 22:03:10
...
EXIT=logs/B.exit
rm -f "$EXIT"
echo "=== roast nominal_run3_quadjet_run2_20260923_15e45e6-8998887 step B start $(date) on $(hostname) ===" | tee -a "$LOG"
...
./run_container snakemake -s coffea4bees/workflows/Snakefile_PhaseB.smk --configfile roasts/nominal_run3_quadjet_run2_20260923_15e45e6-8998887/config.yml --cores 8 --jobs 8 --printshellcmds --config roast_id=nominal_run3_quadjet_run2_20260923_15e45e6-8998887 2>&1 | tee -a "$LOG"
RC=${PIPESTATUS[0]}
echo "=== roast nominal_run3_quadjet_run2_20260923_15e45e6-8998887 step B exit $RC $(date) ===" | tee -a "$LOG"
echo "$RC" > "$EXIT"
if [ "$RC" != "0" ]; then
