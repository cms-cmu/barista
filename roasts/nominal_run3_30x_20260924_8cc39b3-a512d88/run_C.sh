#!/usr/bin/env bash
# roast nominal_run3_30x_20260924_8cc39b3-a512d88 step C -- generated 2026-09-24 21:23:06
...
main() {
cd "$HOME/work/prod/nominal_run3_30x_20260924_8cc39b3-a512d88/barista" || exit 97
LOG=logs/C.log
EXIT=logs/C.exit
rm -f "$EXIT"
echo "=== roast nominal_run3_30x_20260924_8cc39b3-a512d88 step C start $(date) on $(hostname) ===" | tee -a "$LOG"
...
./run_container snakemake -s coffea4bees/workflows/Snakefile_PhaseC.smk --configfile roasts/nominal_run3_30x_20260924_8cc39b3-a512d88/config.yml --cores 4 --jobs 4 --printshellcmds --config roast_id=nominal_run3_30x_20260924_8cc39b3-a512d88 --unlock || true
./run_container snakemake -s coffea4bees/workflows/Snakefile_PhaseC.smk --configfile roasts/nominal_run3_30x_20260924_8cc39b3-a512d88/config.yml --cores 4 --jobs 4 --printshellcmds --config roast_id=nominal_run3_30x_20260924_8cc39b3-a512d88 --rerun-incomplete 2>&1 | tee -a "$LOG"
RC=${PIPESTATUS[0]}
echo "=== roast nominal_run3_30x_20260924_8cc39b3-a512d88 step C exit $RC $(date) ===" | tee -a "$LOG"
echo "$RC" > "$EXIT"
if [ "$RC" != "0" ]; then
