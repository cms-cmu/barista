#!/usr/bin/env bash
# roast nominal_run3_30x_20260924_8cc39b3-a512d88 step C4 -- generated 2026-09-25 08:26:04
...
EXIT=logs/C4.exit
rm -f "$EXIT"
echo "=== roast nominal_run3_30x_20260924_8cc39b3-a512d88 step C4 start $(date) on $(hostname) ===" | tee -a "$LOG"
...
./run_container snakemake -s coffea4bees/workflows/Snakefile_PhaseC_4_FvT_closure.smk --configfile roasts/nominal_run3_30x_20260924_8cc39b3-a512d88/config.yml --cores 8 --jobs 8 --printshellcmds --config roast_id=nominal_run3_30x_20260924_8cc39b3-a512d88 2>&1 | tee -a "$LOG"
RC=${PIPESTATUS[0]}
echo "=== roast nominal_run3_30x_20260924_8cc39b3-a512d88 step C4 exit $RC $(date) ===" | tee -a "$LOG"
echo "$RC" > "$EXIT"
if [ "$RC" != "0" ]; then
