#!/usr/bin/env bash
# roast nominal_run3_30x_20260924_8cc39b3-a512d88 step D -- generated 2026-09-25 08:26:19
...
EXIT=logs/D.exit
rm -f "$EXIT"
echo "=== roast nominal_run3_30x_20260924_8cc39b3-a512d88 step D start $(date) on $(hostname) ===" | tee -a "$LOG"
...
./run_container snakemake -s coffea4bees/workflows/Snakefile_PhaseD.smk --configfile roasts/nominal_run3_30x_20260924_8cc39b3-a512d88/config.yml --cores 4 --jobs 4 --printshellcmds --config roast_id=nominal_run3_30x_20260924_8cc39b3-a512d88 2>&1 | tee -a "$LOG"
RC=${PIPESTATUS[0]}
echo "=== roast nominal_run3_30x_20260924_8cc39b3-a512d88 step D exit $RC $(date) ===" | tee -a "$LOG"
echo "$RC" > "$EXIT"
if [ "$RC" != "0" ]; then
