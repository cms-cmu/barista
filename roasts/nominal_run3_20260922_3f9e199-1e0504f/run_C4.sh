#!/usr/bin/env bash
# roast nominal_run3_20260922_3f9e199-1e0504f step C4 -- generated 2026-09-23 07:34:20
...
EXIT=logs/C4.exit
rm -f "$EXIT"
echo "=== roast nominal_run3_20260922_3f9e199-1e0504f step C4 start $(date) on $(hostname) ===" | tee -a "$LOG"
...
./run_container snakemake -s coffea4bees/workflows/Snakefile_PhaseC_4_FvT_closure.smk --configfile roasts/nominal_run3_20260922_3f9e199-1e0504f/config.yml --cores 8 --jobs 8 --printshellcmds --config roast_id=nominal_run3_20260922_3f9e199-1e0504f 2>&1 | tee -a "$LOG"
RC=${PIPESTATUS[0]}
echo "=== roast nominal_run3_20260922_3f9e199-1e0504f step C4 exit $RC $(date) ===" | tee -a "$LOG"
echo "$RC" > "$EXIT"
if [ "$RC" != "0" ]; then
