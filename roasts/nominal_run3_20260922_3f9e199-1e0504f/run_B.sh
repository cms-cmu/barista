#!/usr/bin/env bash
# roast nominal_run3_20260922_3f9e199-1e0504f step B -- generated 2026-09-22 22:17:14
...
main() {
cd "$HOME/nobackup/HH4b/prod/nominal_run3_20260922_3f9e199-1e0504f/barista" || exit 97
LOG=logs/B.log
EXIT=logs/B.exit
rm -f "$EXIT"
echo "=== roast nominal_run3_20260922_3f9e199-1e0504f step B start $(date) on $(hostname) ===" | tee -a "$LOG"
...
./run_container snakemake -s coffea4bees/workflows/Snakefile_PhaseB.smk --configfile roasts/nominal_run3_20260922_3f9e199-1e0504f/config.yml --cores 8 --jobs 8 --printshellcmds --config roast_id=nominal_run3_20260922_3f9e199-1e0504f --forcerun jcm_cutflow_closure_table 2>&1 | tee -a "$LOG"
RC=${PIPESTATUS[0]}
echo "=== roast nominal_run3_20260922_3f9e199-1e0504f step B exit $RC $(date) ===" | tee -a "$LOG"
echo "$RC" > "$EXIT"
if [ "$RC" != "0" ]; then
